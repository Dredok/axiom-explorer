...
Version: v1.2.2-preprint\\
DOI: \href{https://doi.org/10.5281/zenodo.20184660}{10.5281/zenodo.20184660}}
...
For a topological group $G$, let $\mathrm{wt}(G)$ denote its topological
weight (the minimum cardinality of a basis of open neighbourhoods of the
identity). For free profinite groups, the cardinality bound
$|\Frhat_S| \le 2^{\max(|S|,\aleph_0)}$ (stated as \eqref{eq:Frbound} in
\S\ref{sec:direct}) is the form of the bound we will actually use, with
equality in the cases considered.
...
\kappa_{\mathcal X} \;:=\; \max\!\left(
  ...
  \sup_{x,y \in \pi_0\,\Pt(\mathcal X)(\ast)}
  \mathrm{wt}\bigl(\pi_0\,\Map_{\Pt(\mathcal X)(\ast)}(x,y)\bigr)
\right).
\end{equation}
Here $\mathrm{wt}$ is applied to the natural profinite/condensed topology on
the mapping sets arising from the Galois-category structure of the spectral
$\infty$-topos $\mathcal X$ (cf.\ \cite{BarwickGlasmanHaine2018, BhattScholze2015});
it is \emph{not} the cardinality of the underlying set. We use
$\continuum := 2^{\aleph_0}$ throughout.
\end{convention}

This formulation builds $\kappa_{\mathcal X}$ from data on $\Pt(\mathcal X)$
itself --- the cardinality of its set of components and the topological
weight of its mapping data --- and not from the very fundamental group
$\pi_1$ of the classifying anima that the conjecture aims to bound.
Earlier drafts that took the third term as a $\pi_1$ of $\Pt(\mathcal X)$
were potentially circular; this is fixed here.

The distinction between weight and underlying cardinality is what makes the
envelope non-trivial. A free profinite group on a set $S$ has weight $|S|$ but
cardinality up to $2^{|S|}$; using cardinality in \eqref{eq:kappa} would
force the predicted bound to grow one full beth level beyond what the
geometric attestations of \S\ref{sec:direct} actually realise.
...
\begin{remark}[Reviewer corrections to \eqref{eq:kappa}]
The form of \eqref{eq:kappa} has been adjusted twice in response to
reviewer feedback. The v1 form used $|\Pt(\mathcal X)(\ast)|$ (the bare
underlying set), which is not invariant under equivalence of categories.
The v1.1 fix to $|\pi_0\Pt|$ plus a third term was equivalence-invariant
but used $\sup |\pi_0\Map(x,y)|$ (cardinality of the mapping set), which
for the geometric \'etale $\infty$-topos already absorbs the cardinality of
the absolute Galois group of a generic point and can be as large as
$2^{|k|}$, breaking the attestations of \S\ref{sec:direct}. The v1.2 fix
to $\sup \mathrm{wt}(\pi_1(\Pt(\mathcal X)(\ast),x))$ used the topological
weight but, as a second reviewer noted, this third term is too close to
the very $\pi_1$ that the conjecture intends to bound and was thus
potentially circular. The current v1.2.1 form takes the weight of the
\emph{mapping data} of $\Pt(\mathcal X)$, which is genuinely a property of
$\Pt(\mathcal X)$ as a (condensed) $\infty$-category and not of its
classifying anima.
\end{remark}
...
For $\Pone_{\Q}$, \cite[Remark~7.14]{HHLMMW2025} identifies the relevant
generic Galois group surjecting onto $\pi_1^{\mathrm{cond}}(\Pone_{\Q})(\ast)$
with the free profinite group $\Frhat_{\Q}$, of weight $\aleph_0$ and
cardinality at most $\continuum$. Together with $|\pi_0\Pt(\Pone_{\Q})(\ast)|
= \aleph_0$, this gives $\kappa_{\Pone_{\Q}} = \aleph_0$. The envelope
$2^{\kappa}$ is $\continuum$, and the attested upper bound
...
\cite[Example 4.8]{CampionCousinsYe2021}, a profinite group of cardinality
...
\addlinespace
Analogical (\S\ref{sec:modeltheory}) & $\GalL(\mathrm{ACF}_0) \simeq \Galabs(\overline{\Q}/\Q)$ & $\aleph_0$ & $= \continuum$ (\textbf{attained}) & \cite[Ex.~4.8]{CampionCousinsYe2021} \\
...
\begin{enumerate}
\item \textbf{In the known geometric cases}, the computation factors
through an ambient profinite group attached to $\Pt(\mathcal X)(\ast)$ ---
an absolute Galois or automorphism group of a generic point --- whose
topological weight is at most $\kappa$ (controlled by the third term of
\eqref{eq:kappa}); its underlying cardinality is then at most $2^{\kappa}$
by \eqref{eq:Frbound}. The argument runs through the
weight/generation-size invariant, not through the cardinality of the
mapping spaces in $\Pt(\mathcal X)$.
\item \textbf{Quotienting by an abstract normal closure} can only shrink the
group. Hence $|\pi_1| \le 2^{\kappa}$ \emph{in those cases}.
\end{enumerate}

A proof of Conjecture~\ref{conj:envelope} would require showing that the
weight bound of step (1) holds for spectral $\infty$-topoi in the stated
generality, which we do not claim to have done.
...
\emph{axiom-explorer} \cite{Vera2026Methodology, AxiomExplorerRepo}. Its workflow:
...
The v1.2.1 corrections, prompted by a third round of review of v1.2,
were precision adjustments rather than substantive: replacing the
$\sup \mathrm{wt}(\pi_1(\Pt(\mathcal X)(\ast),x))$ third term of \eqref{eq:kappa}
by $\sup_{x,y} \mathrm{wt}(\pi_0\Map_{\Pt(\mathcal X)(\ast)}(x,y))$ to remove a
potential circularity (the v1.2 form named the very $\pi_1$ that the
conjecture aims to bound); softening the v1.2 claim that profinite
weight equals minimum generation size, in favour of the safer cardinal
bound $|\Frhat_S| \le 2^{\max(|S|,\aleph_0)}$ which is what the
arguments actually require; correcting the cited example numbers in
Campion-Cousins-Ye (Example~4.8 for ACF, not 4.6; Example~4.4 for DLO,
correct in v1.2); rebasing the structural sketch in
\S\ref{sec:sketch} to make explicit that the weight bound is verified
in the geometric attestations, not proved in general; and updating the
$\Pone_{\Q}$ paragraph to use \cite[Remark 7.14]{HHLMMW2025}'s exact
formulation in terms of $\Frhat_{\Q}$.

The v1.2.2 release is the v1.2.1 manuscript with the Zenodo DOI
embedded on the title page and in the machine-readable metadata; no
changes to the mathematical content. v1.2.2 also adds an explicit
citation to the companion methodology preprint
\cite{Vera2026Methodology}, which had not yet received its own DOI at
the time of the v1.2.1 acknowledgments paragraph.
...
\item Cited papers can be re-downloaded by \texttt{curl} from arXiv;
the exact arXiv identifiers are listed in \texttt{references.bib}.
...
This v1.2.1 is intended as a \emph{registered conjectural observation} and an
...
Any of the four is a useful update. We will issue a revised version of this preprint if
