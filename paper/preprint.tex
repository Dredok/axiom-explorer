...
\kappa_{\mathcal X} \;:=\; \max\!\left(
  ...
  \sup_{x,y \in \pi_0\,\Pt(\mathcal X)(\ast)} \!\!|\pi_0\,\Map(x,y)|
\right),
\end{equation}
where $|\cdot|$ on a set returns its cardinality. We use $\continuum := 2^{\aleph_0}$
throughout.
\end{convention}

The conjecture below is invariant under equivalence of categories of points
since each ingredient on the right of \eqref{eq:kappa} is. The third term
controls automorphism complexity at the level of components.

\begin{remark}\label{rem:kappa-coarse}
Convention \ref{conv:kappa} is deliberately coarse. It is plausible that an
even tighter invariant is possible (e.g.\ involving the weight of the relevant
profinite groups). A sharper definition is left open as part of the
conjecture's refinement.
\end{remark}
...
to Campion, Cousins and Ye \cite{CampionCousinsYe2024}; the condensed-group
...
For $\Pone_k$ over an infinite separably closed field $k$, the size invariant
of Convention \ref{conv:kappa} reduces in this case to $\kappa = \max(\aleph_0,
|k|)$ via the absolute Galois group of the generic point and the cardinality
of the set of closed points; we have $\kappa = \continuum$ for $k = \C$ and
$\kappa = \aleph_0$ for $k = \Q$. Both attestations of
Theorem \ref{thm:geomatt} match the envelope $2^{\kappa}$ of
Conjecture \ref{conj:envelope} exactly. The first is attained; the second is
the upper bound the paper proves. (Whether the second is also attained is, as
far as we know, open: it bears on Question \ref{q:saturation}.)
...
The work of Campion, Cousins, and Ye \cite{CampionCousinsYe2024} identifies
$\GalL(T)$ with $\pi_1$ of the geometric realisation of the category of models
$|\mathrm{Mod}_T|$. The condensed-group upgrade of this identification, asserted
in \cite[\S 0]{Haine2026classifying} as joint forthcoming work of Haine, Damaj
and Zhang, would place $\GalL(T)$ inside the framework of
Conjecture \ref{conj:envelope}.

What is true \emph{classically} about $|\GalL(T)|$:

\begin{itemize}
\item For ACF$_0$ (algebraically closed fields of characteristic $0$), in
the formulation without naming constants for $\overline{\Q}$, the group
$\GalL(\mathrm{ACF}_0)$ is isomorphic to the absolute Galois group
$\Galabs(\overline{\Q}/\Q)$, which is a profinite group of cardinality
$\continuum$ \cite[\S 6.1]{Casanovas2011}. This sits inside the envelope
$2^{\kappa} = \continuum$ for $\kappa = \aleph_0$, but does not attain it.
\item For DLO (dense linear orders without endpoints), the theory is
$\omega$-categorical, $\GalL(\mathrm{DLO})$ is trivial \cite{Casanovas2011}.
This sits trivially inside the envelope but does not attain it. (We retract
an earlier claim that DLO is ``simple''; DLO is the prototype of an
\emph{unstable} theory \cite{nLab-DLO}.)
\end{itemize}
...
The model-theoretic analogue of Conjecture \ref{conj:envelope}, conditional on
the announced Haine-Damaj-Zhang condensed-group upgrade, would read: for $T$
of cardinality $\le \continuum$ with monster of cardinality $\kappa$,
$|\GalL(T)| \le 2^{\kappa}$. The classical Lascar group is in any case bounded
above by the cardinality of the monster's automorphism group, which gives a
very weak version of this; sharper bounds are exactly what the conjecture
would predict.
...
\addlinespace
Analogical (\S\ref{sec:modeltheory}) & $\GalL(\mathrm{ACF}_0) \simeq \Galabs(\overline{\Q}/\Q)$ & $\aleph_0$ & $= \continuum$ (within envelope, not attained) & \cite[\S 6.1]{Casanovas2011} \\
Analogical (\S\ref{sec:modeltheory}) & $\GalL(\mathrm{DLO})$ & $\aleph_0$ & trivial (within envelope) & \cite{Casanovas2011} \\
\addlinespace
...
cardinality at most $2^{\max(\kappa, \aleph_0)} = 2^{\kappa}$ by
\eqref{eq:Frbound} applied to a generating set of size at most $\kappa$.
...
the generalisation. For $X = \Pone_k$ with $k$ infinite separably closed,
this lower bound input is realised by the construction of
\cite[\S 7.1]{HHLMMW2025}, which is what makes $\Pone_{\C}$ saturate. Whether
...
of \cite{CampionCousinsYe2024}: a complete theory $T$ for which the condensed
...
We are grateful to two anonymous AI reviewers (one moderately permissive,
one technically severe) whose commentary on draft v1 led to the substantive
corrections embodied in this v1.1: separating the upper bound from the
saturation claim (\S\ref{sec:intro}); correcting the size invariant
$\kappa_{\mathcal X}$ (Convention \ref{conv:kappa}); fixing the cardinality
bound on free profinite groups \eqref{eq:Frbound}; correcting the model-theoretic
discussion of $\GalL(\mathrm{ACF}_0)$ and DLO (\S\ref{sec:modeltheory});
demoting the set-theoretic discussion from ``attestation'' to ``related
cardinal phenomena'' (\S\ref{sec:analogical}); reformulating the
falsifiability section to avoid a near-tautological bound
(\S\ref{sec:falsifiability}); and toning the title from
\emph{cardinality envelope} to \emph{conjectural cardinality envelope}.
...
where classical $\pi_2^{\mathrm{\acute{e}t}}(\mathrm{K3}_{\C}, \mathbf{Z}_l)$
is non-trivial of rank $22$:

\begin{question}[K3 at $n=2$]
Does $|\pi_2^{\mathrm{cond}}(\mathrm{K3}_{\C}, \overline{x})(\ast)|$ attain
$2^{\continuum}$, or does it remain at the classical rank-$22$ structure?
\end{question}
